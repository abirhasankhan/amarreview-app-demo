...
export function useReviews() {
    ...
    async function createReview(reviewData) {
        return handleFetch(async () => {
            ...
            const data = await post("/reviews", reviewData);
    ...
    async function getVoteCount(reviewId) {
        return handleFetch(async () => {
            const { count } = await get(`/reviews/${reviewId}/count`);
            return count;
        });
    }
    ...
    async function toggleHelpfulVote(reviewId) {
        return handleFetch(async () => {
            ...
            const { helpful_votes, user_voted } = await (hasVoted
                ? del(`/reviews/${reviewId}/vote`, {}, { credentials: "include" })
                : post(`/reviews/${reviewId}/vote`, {}, { credentials: "include" }));
    ...
    async function respondToReview(reviewId, content) {
        return handleFetch(async () => {
            ...
            const data = await post(`/reviews/${reviewId}/respond`, { content });
    ...
    async function updateResponse(reviewId, content) {
        return handleFetch(async () => {
            const data = await put(`/reviews/${reviewId}/respond`, { content });
    ...
    async function deleteResponse(reviewId) {
        return handleFetch(async () => {
            await del(`/reviews/${reviewId}/respond`);
