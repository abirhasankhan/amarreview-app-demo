      ...
      <template v-else>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex justify-between items-center mb-4">
            ...
            <NuxtLink :to="`/business/${business?.id}/reviews`" class="text-sm text-emerald-600 hover:text-emerald-700">
              View all
            </NuxtLink>
          </div>
              ...
              <div class="flex items-start justify-between">
                <div class="flex items-center">
                  <img :src="review.user.avatar_url ||
                    ...
                    " :alt="review.user.full_name" class="w-8 h-8 rounded-full" />
                  <div class="ml-3">
...
const slug = route.query.slug // if passed as query param


const { business, loading, error, fetchBusinesses } = useBusiness()
const user = useSupabaseUser()

// Fetch business on mount or if route changes
watchEffect(async () => {
  if (businessId.value) {
    try {
      await fetchBusinesses(businessId.value)
    } catch (err) {
...
const averageRating = computed(() => {
  ...
  const total = business.value.reviews.reduce((sum, r) => sum + r.rating, 0)
